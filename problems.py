...
class RealProblem(Problem):
    ...
    def __init__(self, name, dimension, num_bit_code, lower_bounds, upper_bounds,base):
    ...
    def convertToReal(self, chr):
        ...
        return dis.convertFromBinToFloat(chr, self.lower_bounds, self.upper_bounds, self.num_bit_code, self.dim, self.base)
    ...
    def getOptimum(self):
        ...
        list_all_values = []
        for i in range(self.dim):
            list_all_values.append(dis.getAllValues(self.lower_bounds[i], self.upper_bounds[i], self.num_bit_code, self.base))
        all_space = list(itertools.product(*list_all_values))
        for chr in all_space:
            fitnesses.append(self.computeFitness(chr))
        if self.isMaxProblem():
        ...
        for ind in list_index:
            ...
            for i in range(self.dim):
                l.append(
                    dis.convertFromFloatToBin(all_space[ind][i], self.lower_bounds[i], self.upper_bounds[i], self.num_bit_code, self.base))
    ...
    def getWholeSpace(self):
        ...
        for chr in sols:
            ...
            l = []
            for i in range(self.dim):
                l.append(dis.convertFromFloatToBin(chr[i], self.lower_bounds[i], self.upper_bounds[i], self.num_bit_code, self.base))
            gray_sols.append(l)
...
class RastriginProblem(RealProblem):
    def __init__(self, dim = 2, num_bit_code = 4, lower_bounds = [-5.12,-5.12], upper_bounds = [5.12,5.12], A=10, base=4):
        super().__init__("Rastrigin", dim, num_bit_code, lower_bounds, upper_bounds, base)
        self.A=A
...
class SphereProblem(RealProblem):
    def __init__(self,dim = 2, num_bit_code = 4, lower_bounds = [-5.12,-5.12], upper_bounds = [5.12,5.12], base=4):
        super().__init__("Sphere", dim, num_bit_code, lower_bounds, upper_bounds, base)
...
class BealeProblem(RealProblem):
    def __init__(self,dim = 2, num_bit_code = 4, lower_bounds = [-4.5,-4.5], upper_bounds = [4.5,4.5],base=4):
        super().__init__("Beale", dim, num_bit_code, lower_bounds, upper_bounds, base)
...
class RosenbrockProblem(RealProblem):
    def __init__(self, dim = 2, num_bit_code = 4, lower_bounds = [-2.048,-2.048], upper_bounds = [2.048, 2.048], base=4):
        super().__init__("Rosenbrock", dim, num_bit_code, lower_bounds, upper_bounds, base)
...
class StepProblem(RealProblem):
    def __init__(self, dim = 2, num_bit_code = 4, lower_bounds = [-5.12,-5.12], upper_bounds = [5.12,5.12], base=4):
        super().__init__("Step", dim, num_bit_code, lower_bounds, upper_bounds, base)
...
class QuarticProblem(RealProblem):
    def __init__(self, dim = 2, num_bit_code = 4, lower_bounds = [-1.28,-1.28], upper_bounds = [1.28, 1.28], base=4):
        super().__init__("Quartic", dim, num_bit_code, lower_bounds, upper_bounds, base)
...
class SchwefelProblem(RealProblem):
    def __init__(self,dim = 2, num_bit_code = 4, lower_bounds = [-500,-500], upper_bounds = [500,500], V = 418.9829, base=4):
        super().__init__("Schwefel", dim, num_bit_code, lower_bounds, upper_bounds, base)
        self.V = V
